...
for i in range(folds):
    if counter == 1:
        end_index += section
        globals()['tuple{0}' .format(i)] = tuple(list_main[start_index:end_index])
        counter += 1
    elif counter == folds:
        globals()['tuple{0}' .format(i)] = tuple(list_main[end_index: ])
        counter += 1
    elif counter < folds:
...
for i in range(folds):
    print('Tuple {0}: {1}' .format(i, globals()['tuple{0}' .format(i)]))
